...
def read_masks(file_names):
    imgs=[]
    for img_str in file_names:

        img=cv2.imread(img_str, -1).astype(np.float32)
        imgs.append(img)
...
def hdr_to_ldr(imgs,expo,gamma):
    return np.power(imgs * (expo + 1e-8), 1 / gamma)
...
def batch_ssim(img, imclean):
    ...
    psnr = 0
    for i in range(Img.shape[0]):
        ssim=calculate_ssim(Iclean[i,:,:,:].transpose(1, 2, 0)*255, Img[i,:,:,:].transpose(1, 2, 0)*255)
        psnr += ssim
    return (psnr/Img.shape[0])
def batch_ssim_mu(img, imclean):
    ...
    psnr = 0
    for i in range(Img.shape[0]):
        psnr += calculate_ssim(Iclean[i,:,:,:].transpose(1, 2, 0)*255, Img[i,:,:,:].transpose(1, 2, 0)*255)
    return (psnr/Img.shape[0])
